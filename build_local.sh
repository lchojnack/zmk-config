...
ENV="-e CMAKE_PREFIX_PATH=/zmk/zephyr:${CMAKE_PREFIX_PATH:-} -e GIT_CONFIG_COUNT=1 -e GIT_CONFIG_KEY_0=safe.directory -e GIT_CONFIG_VALUE_0=*"
COMMAND="$RUNTIME run --rm --user $(id -u):$(id -g) --workdir /zmk -v $(pwd):/zmk -v /tmp:/temp $ENV $IMG"
...
# Regenerated by zmk-battery.py on every run (polybar calls it once a minute)
/zmk-battery.png
/zmk-battery.svg
